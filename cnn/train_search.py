...
def get_args():
    parser = argparse.ArgumentParser("cifar arch search")
...
def setup_logging(save):
    log_format = '%(asctime)s %(message)s'
    log_file = os.path.join(save, 'log.txt')
    logging.basicConfig(level=logging.INFO, format=log_format, datefmt='%m/%d %I:%M:%S %p', filename=log_file)
    console = utils.TqdmHandler()
...
def main():
    ...
    for epoch in trange(args.epochs):
        ...
        train_acc, train_obj = train(train_queue, valid_queue, model, architect, criterion, optimizer, lr,
                                     args.report_freq, args.grad_clip, args.unrolled, args.device, args.dtype)
        logging.info('train_acc %f', train_acc)

        # validation
        valid_acc, valid_obj = infer(valid_queue, model, criterion, args.report_freq, args.device, args.dtype)
        logging.info('valid_acc %f', valid_acc)

        utils.save(model, os.path.join(args.save, 'weights.pt'))


def train(train_queue, valid_queue, model, architect, criterion, optimizer, lr, report_freq, grad_clip, unrolled,
          device, dtype):
    ...
    for step, (input, target) in enumerate(tqdm(train_queue)):
        ...
        input_search, target_search = input_search.to(device=device, dtype=dtype), target_search.to(device=device)
        ...
        if step % report_freq == 0:
            logging.info('train (%03d/%03d)\tLoss %.4e\tTop1 %.2f\tTop5 %.2f', step, len(train_queue), objs.avg,
                         top1.avg, top5.avg)

    return top1.avg, objs.avg


def infer(valid_queue, model, criterion, report_freq, device, dtype):
    objs = utils.AvgrageMeter()
    top1 = utils.AvgrageMeter()
    top5 = utils.AvgrageMeter()
    model.eval()
    ...
    for step, (input, target) in enumerate(valid_queue):
        ...
        if step % report_freq == 0:
            logging.info('valid (%03d/%03d)\tLoss %e\tTop1 %.2f\tTop5 %.2f', step, len(valid_queue), objs.avg, top1.avg,
                         top5.avg)
